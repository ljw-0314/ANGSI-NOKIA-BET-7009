...
void pwm_led_mode_set(u8 display)
{
    ...
    if ((display >= _PWM_LED_MODE_END_)|| (display == PWM_LED0_FLASH_TWO) || (display == PWM_LED_MODE_END) || (display == PWM_LED0_FLASH_THREE) || (display == PWM_LED1_FLASH_THREE)/*|| (display == __this->last_mode)*/) {
        return;
    }
