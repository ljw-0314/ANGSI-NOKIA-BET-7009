...
static int frame_fread(struct audio_decoder *decoder, void *buf, u32 len)
{
    ...
    while (len) {
        ...
        if (clen) {
            ...
            if (dec->frame) {
                tws_api_local_media_trans_free(dec->frame);
            }
            dec->frame = tws_api_local_media_trans_pop(&frame_len);
            if (!dec->frame) {
...
static int file_decoder_output_after_syncts(void *priv, void *data, int len)
{
    ...
    if (dec->eq) {
        int eqlen = 0;
        eqlen = audio_eq_run(dec->eq, data, len);
        if (eqlen >= len) {
            dec->remain = 0;
        } else {
            dec->remain = 1;
...
#if TCFG_MUSIC_MODE_EQ_ENABLE
        if (dec->eq) {
            audio_eq_run(dec->eq, data, len);
        }
#endif
#if TCFG_MUSIC_MODE_DRC_ENABLE
        if (dec->drc) {
            audio_drc_run(dec->drc, data, len);
        }
#endif
    }
...
#endif
    return file_decoder_output_after_syncts(dec, data, len);
}
...
static void send_local_media_dec_start_cmd();
static int tws_file_dec_trans_start(void)
{
    ...
    dec->tx_seqn = 1;
    dec->trans_dec.output_handler = tws_local_media_trans_handler;
    ...
    tws_api_local_media_trans_open(10 * 1024);

    send_local_media_dec_start_cmd();
...
static void send_local_media_dec_start_cmd()
{
    ...
    tws_api_local_media_dec_start(args, 4);
}
...
#if TCFG_DEC2TWS_ENABLE
        if (file_dec->frame) {
            tws_api_local_media_trans_free(file_dec->frame);
            file_dec->frame = NULL;
        }
#endif
    }
...
void file_eq_32bit_out(struct file_dec_hdl *dec)
{
    int wlen = 0;
...
static int file_eq_output(void *priv, s16 *buf, u32 len)
{
    int wlen = 0;
    int rlen = len;
    s16 *data = buf;
...
#if MUSIC_EQ_SUPPORT_32BIT
            audio_eq_set_info(eq, eq_param.channels, 1);
#endif

            audio_eq_set_output_handle(eq, file_eq_output, hdl);
            audio_eq_start(eq);
        }
        hdl->eq = eq;
    }
...
#if TCFG_MUSIC_MODE_DRC_ENABLE
    if (!hdl->drc) {
        struct audio_drc *drc = malloc(sizeof(struct audio_drc));
        if (drc) {
            ...
            audio_drc_open(drc, &drc_param);
            audio_drc_set_samplerate(drc, sample_rate);
#if MUSIC_EQ_SUPPORT_32BIT
            audio_drc_set_32bit_mode(drc, 1);
#endif
            audio_drc_set_output_handle(drc, NULL, NULL);
            audio_drc_start(drc);
        }
        hdl->drc = drc;
    }
