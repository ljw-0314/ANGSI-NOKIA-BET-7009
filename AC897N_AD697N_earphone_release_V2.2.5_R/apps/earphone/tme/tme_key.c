...
int tme_tone_get_backup_clk(void);
...
{
    printf("tme tone end callback \n");
    if (tme_tone_get_backup_clk()) {
        clk_set("sys", tme_tone_get_backup_clk());
    }
#if (TCFG_USER_TWS_ENABLE)
    ...
    TME_key_msg_deal(KEY_SEND_SPEECH_START);
}
...
#if TCFG_USER_TWS_ENABLE
int tme_tone_play_reset_clk(u8 index, int flag)
{
...
static void tws_speech_tone_together(void *data, u16 len, bool rx)
{
    ...
    int *arg = (int *)data;
    int msg[8];
    msg[0] = (int)tme_tone_play_reset_clk;
    msg[1] = 2;
    msg[2] = arg[0];
    msg[3] = arg[1];
...
static u16 tme_tone_to_hdl = 0;
static u8 tme_sniff_exit_cnt = 0;
static void tme_start_tone_play(void *priv)
{
    tme_tone_to_hdl = 0;
    if (!bt_is_sniff_close() || (tme_sniff_exit_cnt == 0)) {
        tme_sniff_exit_cnt = 0;
#if(TCFG_USER_TWS_ENABLE)
    ...
    if (tme_sniff_exit_cnt) {
        printf("wait sniff exit:%d \n", tme_sniff_exit_cnt);
        tme_sniff_exit_cnt--;
        tme_tone_to_hdl = sys_timeout_add(NULL, tme_start_tone_play, 100);
    }
}
...
{
    if (tme_tone_to_hdl) {
        sys_timeout_del(tme_tone_to_hdl);
        tme_tone_to_hdl = 0;
    }
    ...
    if (!bt_is_sniff_close()) {
        tme_sniff_exit_cnt = 0;
        tme_tone_to_hdl = sys_timeout_add(NULL, tme_start_tone_play, 10);
        return 0;
    }

    user_send_cmd_prepare(USER_CTRL_ALL_SNIFF_EXIT, 0, NULL);
    tme_sniff_exit_cnt = 13;
    tme_tone_to_hdl = sys_timeout_add(NULL, tme_start_tone_play, 100);

    return (-1);
}

static int tme_key_speech_start(void *arg)
{
#if 0
    ...
    if ((!TME_is_connected()) || (BT_STATUS_WAITINT_CONN == get_bt_connect_status())) {
        return -1;
    }
...
int tme_event_handler(struct sys_event *event)
{
    ...
    case SYS_KEY_EVENT:
        ...
        if (key_event == KEY_SEND_SPEECH_START) {
            printf(">>>>>>>>>>tme key start ai<<<<<<<<<<<<<<<<<<<< \n");
            tme_key_speech_start(event->arg);
            return 1;
        }
        break;
    case SYS_BT_EVENT:
