...
int audio_mic_init(u16 sr)
{
    ...
    mic1_gain = app_var.aec_mic_gain;
    ...
    if (adc_dm) {
        ...
        SFR(JL_ADDA->ADA_CON0,  0,  5,  mic0_gain); // MIC0_PGA_G_11v[4:0]
#endif
        audio_adc_mic1_open(&adc_dm->mic_ch, AUDIO_ADC_MIC_CH, &adc_hdl);
        SFR(JL_ADDA->ADA_CON0,  5,  5, mic1_gain);// MIC1_PGA_G_11v[4:0]
...
static int audio_pdm_mic_init(u16 sr)
{
    ...
    if (plnk_mic) {
        ...
        plnk_mic->sclk_io = PLNK_SCLK_PIN;
        plnk_mic->ch0_io = PLNK_DAT0_PIN;
...
int audio_capture_init(void)
{
    ...
    task_create(AudioCapture_Task, NULL, "audio_capture");
    return 0;
}
