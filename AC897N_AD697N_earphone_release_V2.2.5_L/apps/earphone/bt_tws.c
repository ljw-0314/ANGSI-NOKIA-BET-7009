...
u8 get_ingore_add_pkg(u8 num , u16 check)
{
    return 0;
}
...
{
    puts("idle_tws_remove_pairs\n");
    u8 mac_addr[6];
    char channel = 'U';
    char tws_channel = 0;

    memset(mac_addr,0xFF,6);
    int ret = syscfg_write(CFG_TWS_COMMON_ADDR , (void *)mac_addr ,6);
    if(ret > 0)
    {
        puts("syscfg_write 1 succ\n");
    }
    ret = syscfg_write(CFG_TWS_REMOTE_ADDR, (void *)mac_addr , 6);
    if(ret > 0)
    {
        puts("syscfg_write 2 succ\n");
    }
    // syscfg_read(CFG_BT_MAC_ADDR , mac_addr , 6);
#if (CONFIG_TWS_CHANNEL_SELECT == CONFIG_TWS_LEFT_START_PAIR) || \
    (CONFIG_TWS_CHANNEL_SELECT == CONFIG_TWS_RIGHT_START_PAIR) || \
...
// void check_need_to_clean_info()
// {
//     puts("check_need_to_clean_info\n");

//     if(Long_Press_Reset_Flag & BIT(0))
//     {
//         // need_flag = 0;
//         Long_Press_Reset_Flag &= ~BIT(0);
//         idle_tws_remove_pairs();
