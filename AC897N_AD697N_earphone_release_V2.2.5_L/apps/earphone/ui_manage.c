...
void ui_manage_scan(void *priv)
{
    ...
    if (sys_ui_var.ui_flash_cnt == 0 || sys_ui_var.ui_flash_cnt == 5) {        //有特殊的闪烁状态等当前状态执行完再进入下一个状态
    ...
    case STATUS_POWEROFF:
        ...
        if (p_led->power_off != PWM_LED0_FLASH_TWO) {
    ...
    case STATUS_BT_TWS_DISCONN:
        ...
        /* 
            ****************BET-7009**************
         */
    // case STATUS_TWS_PAIRING:
    //     log_info("[STATUS_TWS_PAIRING]\n");
    //     pwm_led_mode_set(p_led->tws_pairing);
    //     break;
    case STATUS_MUSIC_PAUSE:
        log_info("[STATUS_MUSIC_PAUSE]\n");
        pwm_led_mode_set(p_led->music_pause);
        break;
    case STATUS_MUSIC_PRIVIOUS:
        log_info("[STATUS_MUSIC_PRIVIOUS]\n");
        pwm_led_mode_set(p_led->music_privious);
        break;
    case STATUS_MUSIC_NEXT:
        log_info("[STATUS_MUSIC_NEXT]\n");
        pwm_led_mode_set(p_led->music_next);
        break;
    case STATUS_SIRI:
        log_info("[STATUS_SIRI]\n");
        pwm_led_mode_set(p_led->siri);
        break;
    case STATUS_HANDUP:
        log_info("[STATUS_HANDUP]\n");
        pwm_led_mode_set(p_led->phone_handup);
        break;
    case STATUS_3_PARTY_COMFERENCE:
        log_info("[STATUS_3_PARTY_COMFERENCE]\n");
        pwm_led_mode_set(p_led->_3_party_comference);
        break;
        /* 
            *******************************************
         */
    case STATUS_ENTER_DUT:
        log_info("[STATUS_ENTER_DUT]\n");
...
void ui_update_status(u8 status)
{
    ...
    if ((status == STATUS_POWERON && p_led->power_on == PWM_LED0_FLASH_THREE) || (status == STATUS_POWEROFF && p_led->power_off == PWM_LED0_FLASH_TWO)) {
        log_info(">>>set ui_flash_cnt");
        sys_ui_var.ui_flash_cnt = 5;
    }
    ...
    /* } */
    if (sys_ui_var.ui_flash_cnt >= 1 && sys_ui_var.ui_flash_cnt <= 4) {
        return;
    }
